...
int main()
{
	...
	// scene.add(std::make_shared <Sphere> (Vec3( -1.0,   0.0,    -1.0),   0.4, mat_bubble));
	// scene.add(std::make_shared <Sphere> (Vec3( 1.0,    0.0,    -1.0),   0.5, mat_right));
	//
	// // std::shared_ptr <Lambertian> mat_left  = std::make_shared <Lambertian> (Color(0, 0, 1));
	// // std::shared_ptr <Lambertian> mat_right = std::make_shared <Lambertian> (Color(1, 0, 0));
	// //
	// // scene.add(std::make_shared <Sphere> (Vec3(-R, 0, -1), R, mat_left));
	// // scene.add(std::make_shared <Sphere> (Vec3( R, 0, -1), R, mat_right));
	...
	cam.img_width           = 1200;
	cam.samp_per_pix        = 500;
	cam.max_ray_depth       = 50;
	cam.vert_fov            = 20;
