...
bool Dielectric::scatter_light (const Ray& r_in, const IntersectionInfo& info, Color& attenuation, Ray& scattered) const
{
    ...
    const double c_refr_index = info.orientation ? ( 1.0 / this->refr_index ) : this->refr_index;

    const Vec3   unit_dir  = unit_vec ( r_in.direction () );
    const double cos_theta = std::fmin ( dot_prod ( -unit_dir, info.norm ), 1 );
    const double sin_theta = std::sqrt ( 1.0 - cos_theta * cos_theta );

    const bool uncorrectable = c_refr_index * sin_theta > 1.0;

    // const Vec3 dir = uncorrectable || this->reflectance ( cos_theta, c_refr_index ) > rand_double () ? reflect ( unit_dir, info.norm ) : refract ( unit_dir, info.norm, c_refr_index );
    const bool reflect_now = uncorrectable || this->reflectance ( cos_theta, c_refr_index ) > rand_double ();
    const Vec3 dir         = reflect_now
...
double Dielectric::reflectance (const double cos, const double refr_index)
{
