...
void Camera::init ()
{
	...
	double viewport_height  =       2 * h * this->focus_dist;
	...
	Vec3 viewport_top_left  =       this->cam_center - (this->focus_dist * this->w) - viewport_u / 2 - viewport_v / 2;
	this->pix_sp_loc        =       viewport_top_left + (this->pix_dlt_u + this->pix_dlt_v) * 0.5;

	double defocus_radius   =       this->focus_dist * std::tan(deg_to_rad(this->defocus_ang / 2));
...
Ray Camera::get_ray (const int height, const int width) const
{
	...
	Vec3 ray_origin = this->defocus_ang <= 0 ? this->cam_center : this->defocus_disk_samp();
...
Color Camera::ray_clr (const Ray& r, int depth, const Intersectable& scene) const
{
	...
	// Added 0.001 instead of 0.0 to guard against floating point imprecision
	if (scene.hit(r, Range(0.001, INF), info))
	{
		// OLD^2:
		...
		{
			return attenuation * ray_clr(scattered, --depth, scene);
		}
