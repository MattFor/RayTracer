...
inline Vec3 rand_in_unit_sphere ()
{
    while ( true )
    {
        if ( Vec3 v = Vec3::rand ( -1.0, 1.0 ); v.len_squared () < 1.0 )
        {
            return v;
        }
    }
...
{
    const Vec3 on_unit_sphere = rand_unit_on_hemisphere ();
...
inline Vec3 refract (const Vec3& uv, const Vec3& norm, const double refr_index_ratio)
{
    /**
    ...
    const double cos_theta     = std::fmin ( dot_prod ( -uv, norm ), 1 );
    const Vec3   r_out_perp    = refr_index_ratio * ( uv + cos_theta * norm );
    const Vec3   parallel_comp = -std::sqrt ( std::fabs ( 1.0 - r_out_perp.len_squared () ) ) * norm;
...
inline Vec3 rand_in_unit_disk ()
{
    while ( true )
    {
        if ( auto point = Vec3 ( rand_double ( -1, 1 ), rand_double ( -1.0, 1.0 ), 0.0 ); point.len_squared () < 1.0 )
        {
            return point;
        }
    }
