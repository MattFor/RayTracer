...
public:
    ...
    bool     scatter_light (const Ray& r_in, const IntersectionInfo&, Color& attenuation, Ray& scattered) const override;
...
{
public:
    Metal (const Color& albedo, const double fuzz) : fuzz ( fuzz < 1 ? fuzz : 1 ),
                                                     albedo ( albedo ) {};
    bool scatter_light (const Ray& r_in, const IntersectionInfo&, Color& attenuation, Ray& scattered) const override;
...
{
public:
    explicit Dielectric (const double refr_index) : refr_index ( refr_index ) {}
    bool     scatter_light (const Ray& r_in, const IntersectionInfo&, Color& attenuation, Ray& scattered) const override;
